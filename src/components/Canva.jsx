...
export default class Canva extends React.Component {
    ...
    handleMouseDown(e) {
        this.setState({
            ...
            lastX: this.state.mouseX,
            lastY: this.state.mouseY,
            mouseIsDown: true
        })
    ...
    handleMouseMove(e) {
        ...
        for (var i = 0; i < this.state.ships.length; i++) {
            ...
            if (this.state.ctx.isPointInPath(this.state.lastX, this.state.lastY)) {
                console.log('CLICOU')
                ship.x += (this.state.mouseX - this.state.lastX);
                ship.y += (this.state.mouseY - this.state.lastY);
    ...
    render() {
        return (
            <div>
                ...
                <button onClick={this.drawAllShips}>Desenhar</button>
            </div>
